...
class LoadingSpinner extends Component {
  ...
  render() {
    ...
    }else if ( this.props.parentFlag == true ) {
    ...
    } else if ( this.props.loadingSpinnerState == true ) {
      console.log('loading spinner : line 24: Turned spinner on based on Redux state flag ');      
      return (
        <View>
          <Spinner visible={true} color={'#003399'} size={'large'} />
        </View>
...
const mapStateToProps = ({ auth }) => {
  const { loginStatus ,loadingSpinnerState } = auth;
  return { loginStatus ,loadingSpinnerState};
};
